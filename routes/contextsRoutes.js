...
// GET CONTEXTS CREAETD BY USER
contextRouter.get('/subscribed/:user_id', async(req, res) =>{
    var response = await Contexts4Users.find({user: req.params.user_id})
    res.send(response) 
})

// GET CONTEXTS CREAETD BY USER
contextRouter.get('/created/:user_id', async(req, res) =>{
    var response = await Context.find({user: req.params.user_id})
    res.send(response) 
})

// GET OTHERS CONTEXTS NOT CREAETD NOOR SUBSCRIBED BY USER
contextRouter.post('/otherscontexts', async(req, res) =>{
    var response = await Context.find({"context": { $nin: req.body }})
    res.send(response) 
})

// GET USER SCOPED CONTEXTS
contextRouter.get('/scoped/:context/username', async (req, res)=>{
    // 1) catch conditions and username sent in body
    var context = Context.find({"context": req.params.context})   
    var conditions = req.body.context.condition;
...
// GET ALL CONTEXTS
contextRouter.get('/:limit', async(req, res) =>{
    const { limit } = req.params;
    var response;
    limit ? response = await Context.find().limit(limit) : response = await Context.find();
    res.send(response) 
})
